...
async def scrape_product_url(url: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Amazon URLs cannot be scraped reliably from datacenter IPs (anti-bot
        # strips the buy-box block) and Keepa API access is paywalled. Skip
        # them — they're filtered out of new Google Shopping results, and
        # existing entries can be excluded via the UI.
        if 'amazon.' in domain:
            # If a Keepa API key is configured, try that as a graceful fallback.
            ...
            result["error"] = "Amazon is not supported as a price source."
            return clean_result(result, url)
